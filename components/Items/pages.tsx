...
export default function Items({ title, products }: ItemsProps) {
  ...
  return (
    <div className="flex w-full mt-12 md:mt-20  py-12 md:py-20 overflow-hidden flex-col gap-y-4">
      {/* Heading */}
      <div className="text-3xl md:text-5xl font-light tracking-wide text-black text-center">
        {title}
      </div>
      ...
      <div className="px-4 md:px-16 py-6">
        ...
        <div className="flex gap-4 overflow-x-auto md:hidden">
          {products.map((product, index) => (
            <div key={index} className="flex-shrink-0 w-80 flex flex-col gap-y-2">
              <div className=" rounded-3xl  w-80 flex flex-col px-4">
        ...
        <div className="hidden md:grid gap-x-4 gap-y-6 grid-cols-1 md:grid-cols-3 xl:grid-cols-4">
          {products.map((product, index) => (
            <div key={index} className="flex flex-col max-w-80 gap-y-2">
              <div                   onClick={() =>
                    product.router ? router.replace(product.router) : null
                  }
  className=" rounded-3xl  w-80 max-h-80 flex flex-col px-4">
                <Image
                  src={product.img}
                  alt="product-image"
                  height={300}
